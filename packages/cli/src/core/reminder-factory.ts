...
const TDD_STATE_GOALS: Record<TDDState, string> = {
  [TDDState.EXPLORING]:
    'Understand the architecture and locate the source of the bug. Identify relevant test files.',
  [TDDState.WRITING_TEST]:
    'Write a minimal test case that executes the buggy code path. The goal is for this test to FAIL (RED).',
  [TDDState.REPRO_FAILED]:
    'Bug confirmed (RED). Analyze the failure and implement the minimal necessary fix.',
  [TDDState.WRITING_FIX]:
    'Implement the code changes. Run tests repeatedly until they pass (GREEN).',
  [TDDState.FIX_VERIFIED]:
    'Fix verified (GREEN). Enter CLEANUP phase: revert temporary tests or debug code.',
  [TDDState.CLEANUP]:
    'Ensure all temporary changes are reverted and tests still pass. Generate the final patch.',
};
