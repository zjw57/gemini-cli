    ...
    try {
      ...
      if (subagentTestingConfig.invocationMode === 'heuristic') {
        ...
        if (subAgentName === 'contextHarvester') {
          ...
          if (result.llmContent) {
            (currentMessages[0].parts as Part[]).push({
              text: `The user Ran the tool '${subAgentTool.name}' in order to gather context for you to start your work. 
                The description of the tool is '${subAgentTool.description}'. 
                Bellow, you will find the output of the tool. 
                Use the output of the tool as the base to start of your work so you should avoid redundant thinking.
                Notice that the content of the files are already up-to-date so you do not need to re-read them. 
                The RelevantLocations are locations that are identified as relevant to solve the problem either because they need to be modified or because they will support your modifications.
                <tool_result>
