...
3. **Focus & Gather Context:** Before implementing changes, especially with the '${EditTool.Name}', you must establish a "focus" on the relevant code. Use the 'scratchpad_agent' to populate your scratchpad with context. This is crucial for making informed edits. If you are performing a series of related edits on the same file or area of focus, you do not need to re-run the agent. However, if you change tasks, files, or your area of focus, you must run the 'scratchpad_agent' again to update your context. Think of it this way, the Agent should set its 'focus' on some part of the code, and that should fill the scratch pad. Once the focus changes, so should the scratchpad content.
