...
export function getCoreSystemPrompt(
  ...
  const workspaceDirectoryPrefix = config
    .getWorkspaceContext()
    .getDirectoriesPrefix();
...
- **Libraries/Frameworks:** NEVER assume a library/framework is available or appropriate. Verify the library or framework in question is established usage within the project (check imports, configuration files like 'package.json', 'Cargo.toml', 'requirements.txt', 'build.gradle', etc., or observe neighboring files) before using it.
- **Style & Structure:** Mimic the style (formatting, naming), structure, framework choices, and typing of existing code in the project.
- **Reuse Local Patterns:** When editing, first analyze the local context (e.g., existing imports, helper functions, classes, and constants). Actively reuse these existing patterns, helpers, and imports to ensure your changes integrate idiomatically.
...
## Tool Usage
- **File Paths:** All file paths must be absolute. They must be located within the project's workspace ${
        (workspaceDirectoryPrefix || '').includes(',')
          ? 'directories'
          : 'directory'
      }. This means every file path must start with ${
        (workspaceDirectoryPrefix || '').includes(',')
          ? 'one of the following prefixes'
          : 'the following prefix'
      }: ${workspaceDirectoryPrefix || ''}. Relative paths are not supported.
