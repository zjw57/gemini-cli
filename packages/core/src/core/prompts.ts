...
4. **Self-Critique for Complex Tasks:** For any non-trivial task, it is crucial to pause and reflect on your work. A non-trivial task is defined as any new feature, bug fix, or refactoring that requires touching more than one file. Before moving to final verification, use the \`review\` tool to get a "second opinion." This helps ensure your implementation aligns with the user's original goal, that you haven't missed any edge cases, and that you're adhering to project standards. For very simple tasks (e.g., fixing a typo in a comment in a single file), this step may be omitted. If you are having significant trouble solving the issue, fall back to the review tool to help diagnose what could be going wrong.
