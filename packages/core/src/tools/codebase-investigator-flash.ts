    ...
    "relevant_locations": [
        {
            ...
            "reasoning": "A factual, objective statement explaining *why* this file is relevant to the task (e.g., 'This file defines the Config type' or 'This file passes the config object to the BuiltinCommandLoader'). **DO NOT include any analysis, judgment, or conclusions.**"
...
- **DO:** Find relevant files and provide **objective, factual justification** for their relevance.
- **DO NOT:** Attempt to solve the user's task, write code, or suggest implementations.
- **DO NOT: FORM OR STATE CONCLUSIONS.** Your purpose is to *find evidence*, not to *analyze* it or form an opinion. The central agent will perform the final analysis.
- **DO NOT:** Use judgmental language like "is necessary," "is correct," "is unneeded," or "this confirms." You must report facts, not judgments.
    - **Bad (Judgment):** "This function \`old_api_call\` is obsolete and can be removed."
    - **Good (Fact):** "A codebase grep for \`old_api_call\` returned no usages in the \`src/\` directory."
...
export interface CodebaseInvestigatorOutput {
  ...
  relevant_locations: Array<{
    file_path: string;
    /** A factual, objective statement explaining *why* this file is relevant to the task. */
    reasoning: string;
    key_symbols: string[];
    content?: string;
  }>;
...
> {
  ...
  populateContextState(contextState: ContextState): void {
    ...
    contextState.set(
      'initial_context',
      this.params.initial_context || 'No initial context provided.',
    );
...
export class CodebaseInvestigatorFlashTool extends BaseDeclarativeTool<
  CodebaseInvestigatorInput,
  ToolResult
> {
  ...
  constructor(private config: Config) {
    super(
      CodebaseInvestigatorFlashTool.Name,
