 ...
 */
export interface CodebaseInvestigatorWithFilesInput {
 ...
 */
export interface CodebaseInvestigatorWithFilesOutput {
...
class CodebaseInvestigatorWithFilesInvocation extends BaseToolInvocation<
  CodebaseInvestigatorWithFilesInput,
  ToolResult
> {
  constructor(
    private config: Config,
    params: CodebaseInvestigatorWithFilesInput,
  ) {
    super(params);
  }
  ...
  async execute(): Promise<ToolResult> {
    ...
    const outputConfig: OutputConfig = {
      outputs: {
        report_json:
          'The final JSON report structured according to the CodebaseInvestigatorWithFilesOutput schema.',
      },
    };
    ...
    if (terminate_reason === SubagentTerminateMode.GOAL && reportJson) {
      try {
        const report = JSON.parse(reportJson) as CodebaseInvestigatorWithFilesOutput;
...
export class CodebaseInvestigatorWithFilesTool extends BaseDeclarativeTool<
  CodebaseInvestigatorWithFilesInput,
  ToolResult
> {
  static readonly Name = 'codebase_investigator_with_files';

  constructor(private config: Config) {
    super(
      CodebaseInvestigatorWithFilesTool.Name,
      'Codebase Investigator With Files',
  ...
  protected createInvocation(
    params: CodebaseInvestigatorWithFilesInput,
  ): ToolInvocation<CodebaseInvestigatorWithFilesInput, ToolResult> {
    return new CodebaseInvestigatorWithFilesInvocation(this.config, params);
  }
}
