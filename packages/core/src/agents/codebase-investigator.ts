...
// Define a new type for the report *after* we've added file content to it.
// This helps with type safety in the processing functions.
type Report = z.infer<typeof CodebaseInvestigationReportSchema>;
type LocationWithContent = Report['RelevantLocations'][0] & {
  content?: string;
};
type ReportWithContent = Omit<Report, 'RelevantLocations'> & {
  RelevantLocations: LocationWithContent[];
};

/**
 * Converts the final, content-augmented report object into a structured XML string.
 * @param report The report object, including file content.
 * @returns A formatted XML string.
 */
function convertReportToXmlString(report: ReportWithContent): string {
  // Map each location object to its XML representation.
  const locationsXml = report.RelevantLocations.map((location) => {
    // Map each key symbol to a <Symbol> tag.
    const keySymbolsXml = location.KeySymbols.map(
    ...
    return `  <Location>
    <FilePath>${location.AbsoluteFilePath}</FilePath>
...
${contentXml}  </Location>`;
  ...
  // Map each item in the exploration trace to its own <Trace> tag for clarity.
  const explorationTraceXml = report.ExplorationTrace.map(
    (trace) => `    <Trace>${trace}</Trace>`,
  ).join('\n');

  // Assemble the final report.
  return `
   This is the report generated by the CodebaseInvestigator.
   The RelevantLocations are locations that are identified as relevant to solve the problem either because they need to be modified or because they will support your modifications.
    Notice that the content of the files should already up-to-date. 
  <CodebaseReport>
  <SummaryOfFindings>${report.SummaryOfFindings}</SummaryOfFindings>
  <ExplorationTrace>
${explorationTraceXml}
  </ExplorationTrace>
  <RelevantLocations>
...
> = {
  ...
  description: `Your primary tool for multifile search tasks and codebase exploration. 
    ...
    Returns a structured XML report with key file paths, symbols, and their content to solve a task.`,
  inputConfig: {
    inputs: {
      objective: {
  ...
  outputConfig: {
    outputName: 'report',
    description: 'The final investigation report as an XML object.',
    schema: CodebaseInvestigationReportSchema,
  },

  /**
   * Processes the raw JSON output from the model. This function reads the content
   * of the files identified in the report and then converts the augmented report
   * into a final XML string.
   */
  processOutput: async (output) => {
    // Create a new report object by augmenting the original with file content.
    const reportWithContent: ReportWithContent = {
      ...output,
      RelevantLocations: await Promise.all(
        output.RelevantLocations.map(async (location) => {
          try {
            // Asynchronously read the file content for each location.
            ...
            // Make the process robust. If a file cannot be read, log a warning
            // and proceed without its content.
            console.warn(
              `[Codebase Investigator] Could not read file content for: ${location.AbsoluteFilePath} with error ${error}.`,
            );
            return location; // Return the original location object without content.
          }
        }),
      ),
    };

    // Convert the final, augmented report object to an XML string.
    return convertReportToXmlString(reportWithContent);
  },
