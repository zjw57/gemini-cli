  ...
  it('should automatically compress on long conversation', async () => {
    await rig.setup('automatic compression with low threshold', {
      settings: {
        model: {
          chatCompression: {
            contextPercentageThreshold: 0.01,
          },
        },
      },
    });

    // This prompt should be enough to exceed the 1% threshold.
    const res1 = await rig.run(
    ...
    const res2 = await rig.run('Return another long paragragh.');
